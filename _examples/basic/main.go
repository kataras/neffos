...
func (u userMessage) serialize() []byte {
	v, _ := json.Marshal(u)
	return v
}

func deserialize(b []byte) (u userMessage) {
	json.Unmarshal(b, &u)
	return
}
...
askRoom:
	...
	for {
		...
		// room.String() returns -> room.NSConn.String() returns -> room.NSConn.Conn.String() returns -> room.NSConn.Conn.ID()
		// which generated by server-side via `Server#IDGenerator`.
		userMsg := userMessage{From: username, Text: text}
		room.Emit("chat", userMsg.serialize())
