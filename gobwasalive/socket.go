	...
	for {
		if timeout > 0 {
			if err := s.UnderlyingConn.SetReadDeadline(time.Now().Add(timeout)); err != nil {
				log.Fatalf("gobwasalive Socket SetReadDeadline, err: %v", err)
			}
			if timeout-3*time.Second < s.idleTime {
				delayTime = timeout - 3*time.Second
			}
		}
		...
		if delayTime > 0 {
			delayTimer = s.tw.AfterFunc(delayTime, func() {
				...
				if err := s.write([]byte("ping"), gobwas.OpPing, 3*time.Second); err != nil {
					log.Fatalf("gobwasalive Socket Send keepalive websocket PING, err: %v", err)
				}
			})
		}
